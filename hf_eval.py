...
# ---------- helpers ----------------------------------------------------------
YESNO_RE  = re.compile(r"\b(YES|NO)\b", re.I)     #   case-insensitive literal YES/NO
...
def shuffle_rules(nl_prompt: str) -> str:
    ...
    for i, ln in enumerate(lines):
        if not ln.strip() and i + 1 < len(lines) and \
           lines[i + 1].startswith("And the following facts:"):
            split = i
            break
    if split is None:
        return nl_prompt
...
def parse_answer(text: str) -> tuple[str, str]:
    """
    Extract the first YES/NO token and return (answer, remainder_as_rationale).
    If nothing found -> ("ERROR", original_text)
    """
...
def run(model_id: str,
    ...
    instr = ("Answer exactly YES or NO, then a colon and one-line rationale.\n\n"
             "### Question:\n")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--shuffle_rules", action="store_true",
